...
export default function SecondaryButton({
  ...
  return (
    <button
      type="button"
      className={twMerge(
        "group relative flex items-center justify-center cursor-pointer w-full h-10 rounded-full overflow-hidden p-[1px]",
        variant === "pink"
...
bg-[length:200%_100%] opacity-0 transition-opacity duration-500 ease-in-out animate-[shimmer_2.5s_linear_infinite] group-hover:opacity-100"
      ...
      <span
        className={`relative z-10 text-transparent bg-clip-text ${variant === "pink" ? "bg-[linear-gradient(125deg,_#FFF_22.37%,_#FFB5E6_94.04%)]" : variant === "white" ? "bg-white" : "bg-black"}`}
      >
        {children}
      </span>
    </button>
